...
fn wgc_capture_window(
    ...
    let (sender, receiver) = channel::<XCapResult<RgbaImage>>();

    frame_pool.FrameArrived(
        &TypedEventHandler::<Direct3D11CaptureFramePool, IInspectable>::new({
            move |frame_pool, _| {
                let _ = sender.send(process_frame_arrival(frame_pool, 0, 0, width, height));
                Ok(())
            }
        }),
    )?;
    ...
    if let Err(error) = session.SetIsBorderRequired(false) {
        log::debug!("SetIsBorderRequired(false) failed (non-fatal): {:?}", error);
    }
...
pub(super) fn capture_window(hwnd: HWND) -> XCapResult<RgbaImage> {
    let capture_full_item = |item: &GraphicsCaptureItem| -> XCapResult<RgbaImage> {
        let size = item.Size()?;
        if size.Width <= 0 || size.Height <= 0 {
            return Err(XCapError::new(
                "window GraphicsCaptureItem has an invalid size",
            ));
        }
        wgc_capture_window(item, size.Width as u32, size.Height as u32)
    };
    let interop = factory::<GraphicsCaptureItem, IGraphicsCaptureItemInterop>()?;
    let mut window_items = WINDOW_GRAPHICS_CAPTURE_ITEM.lock()?;
    let key = hwnd.0 as usize;
    if let Some(item) = window_items.get(&key) {
        capture_full_item(item)
    } else {
