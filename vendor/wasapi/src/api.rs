...
impl Handle {
    ...
    pub fn wait_for_event(&self, timeout_ms: u32) -> WasapiRes<()> {
        ...
        if retval == WAIT_OBJECT_0 {
            ...
            Err(WasapiError::UnexpectedWaitResult(retval.0))
        }
    }
}
...
mod wait_tests {
    ...
    #[test]
    fn failed_windows_wait_is_not_reported_as_timeout() {
        // A null HANDLE makes WaitForSingleObject return WAIT_FAILED. This
        // exercises the real Windows boundary without requiring audio hardware.
        let invalid = Handle {
        ...
        let error = invalid
            .wait_for_event(0)
            .expect_err("an invalid wait handle must fail");
